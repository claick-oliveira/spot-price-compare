...
parser.add_argument('--region', type=str,
                    help='Region\'s code, like us-east-1', required=True)
parser.add_argument('--product-type', type=str, default='Linux/UNIX', help='(default: %(default)s)', choices=[
...
parser.add_argument('--mode', type=str, default='show',
                    help='(default: %(default)s)', choices=['show', 'compare'])
parser.add_argument('--output', type=str, default='text',
                    help='(default: %(default)s)', choices=['text', 'json', 'table'])
parser.add_argument('--price-type', type=str, default='all',
                    help='(default: %(default)s)', choices=['all', 'ecu', 'memory', 'economy'])
parser.add_argument('--instance-type', nargs='+',
                    help='(t1.micro or t1.micro t2.micro ...)', default=[], required=True)
...
table = PrettyTable()
table.field_names = ['instance type', 'Availability Zone', 'OnDemand Price',
                     'Spot Price', 'Percentage Economy', 'Price per Memory', 'Price per ECU']
...
for instanceType in args.instance_type:
    ...
    if (responseActualPrice['PriceList'] != []):
      ...
      data_key = list(data['terms']['OnDemand'].keys())[0]

      terms_key = list(data['terms']['OnDemand'][data_key]
                      ['priceDimensions'].keys())[0]
      terms = data['terms']['OnDemand'][data_key]['priceDimensions'][terms_key]
      ...
      if ("On Demand" in terms['description'] and "BYOL" not in terms['description']) or ("Unused Reservation" in terms['description']):
        ...
        for spotPrice in responseSpotPrice['SpotPriceHistory']:
          ...
          if args.mode == 'compare':
              if float(payload['economy']) > float(betterPrice['economy']):
                  betterPrice = payload
              if float(payload['pricePerMemory']) < float(betterMemoryPrice['pricePerMemory']):
                  betterMemoryPrice = payload
              if float(payload['pricePerEcu']) < float(betterECUPrice['pricePerEcu']):
              ...
              if args.output == 'text':
                  ...
                  table.add_row([payload['type'], payload['AvailabilityZone'], payload['actualPrice'],
                              payload['spotPrice'], payload['economy'], payload['pricePerMemory'], payload['pricePerEcu']])
              else:
...
if args.mode == 'compare' and args.output == 'text':
  ...
  table.add_row([betterPrice['type'], betterPrice['AvailabilityZone'], betterPrice['actualPrice'],
                 betterPrice['spotPrice'], betterPrice['economy'], betterPrice['pricePerMemory'], betterPrice['pricePerEcu']])
  table.add_row([betterMemoryPrice['type'], betterMemoryPrice['AvailabilityZone'], betterMemoryPrice['actualPrice'],
                 betterMemoryPrice['spotPrice'], betterMemoryPrice['economy'], betterMemoryPrice['pricePerMemory'], betterMemoryPrice['pricePerEcu']])
  table.add_row([betterECUPrice['type'], betterECUPrice['AvailabilityZone'], betterECUPrice['actualPrice'],
                 betterECUPrice['spotPrice'], betterECUPrice['economy'], betterECUPrice['pricePerMemory'], betterECUPrice['pricePerEcu']])
